...
function updateSpaWalls(spa) {
  ...
  // --- Water ---
  // Use a single horizontal water surface like the pool water so the spa
  // matches the pool visually instead of rendering as a tinted water box.
  const poolWaterTop = SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST;

  // Keep the spa water surface slightly inside the inner wall faces so no
  // vertical water "box" reads around the perimeter.
  const waterInset = Math.max(0.02, t * 0.5 + overflow);

  water.scale.set(
    Math.max(0.01, l - waterInset * 2.0),
    Math.max(0.01, w - waterInset * 2.0),
    1
  );

  const waterSurfaceLocalZ = poolWaterTop - spa.position.z;
  water.position.set(0, 0, waterSurfaceLocalZ);

  // Force the shared pool-style water shader to render as a flat horizontal
  // surface only.
...
export function snapToPool(spa) {
  ...
  if (close === "left") {
    ...
    isHalfwayInWall = Math.abs(x - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    spa.position.x = isHalfwayInWall ? centerPos : innerPos;
  ...
  if (close === "front") {
    ...
    isHalfwayInWall = Math.abs(y - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    spa.position.y = isHalfwayInWall ? centerPos : innerPos;
...
// Water (reuse pool water system as a flat surface, same as the pool)
const water = createPoolWater(1, 1);
water.userData.isSpaWater = true; // so PBR won't tile over this
water.name = "SpaWaterSurface";

// Make sure no legacy volumetric spa water mesh survives on rebuilds.
spa.children
  .filter((child) => child?.userData?.isLegacySpaWaterBox)
  .forEach((child) => {
    spa.remove(child);
